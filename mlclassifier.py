...
with open('data/combo.txt','r') as f:
	...
	for mailbox in mails:
		#print ()
		#break
		for mail in mails[mailbox]:
			data = {}
			data['to'] = mailbox.lower()+'@mailinator.com'
			data['from_'] = mail['senderEmail']
			data['body'] = mail['msg_body']
			allMails.append(data)
print(len(allMails))
email_df = pd.DataFrame(allMails)
...
vect = TfidfVectorizer(encoding ='utf-8',  analyzer='word', stop_words=ENGLISH_STOP_WORDS, max_df=0.3, min_df=2)
